...
#define FRT_LFT_REFL_IR_PORT in5
#define FRT_RGT_REFL_IR_PORT in6
#define FRT_LFT_REFL_IR_THRESHOLD 2100
#define FRT_RGT_REFL_IR_THRESHOLD 1300
#define BCK_LFT_REFL_IR_PORT in8
#define BCK_RGT_REFL_IR_PORT in7
...
void run_machine()
{
	...
	case MOVE_TO_CENTER:
		...
		if (time1[T1] > 3000)
		{
			send_debug_msg("Changing to SEARCH_FOR_BALL", 48);
	...
	case MOVE_TO_BALL:
		...
		case OFFSET:
			...
			if (time1[T1] > correction_time(lowest_reading))
			{
		...
		case MOVE_TOWARDS_BALL:
			movement(STRAIGHT, 100);
			if (time1[T1] > correction_time(lowest_reading) && !(detect_ball_collector()) ) //*****
			{
				machine_mode = SEARCH_FOR_BALL;
				clearTimer(T1);
			}
...
task main()
{
	...
	//writeDebugStreamLine("front left: %d front right: %d ", SensorValue(FRT_LFT_REFL_IR_PORT), SensorValue(FRT_RGT_REFL_IR_PORT) );
	//writeDebugStreamLine("back left: %d back right: %d ", SensorValue(BCK_LFT_REFL_IR_PORT), SensorValue(BCK_RGT_REFL_IR_PORT) );
	//writeDebugStreamLine("switch is %d", SensorValue(START_SWITCH_PORT));
	//writeDebugStreamLine("Door Limit: %d", SensorValue(DOOR_LSWITCH_PORT));
	//writeDebugStreamLine("Ball Limit: %d", SensorValue(BALL_LSWITCH_PORT));
	//writeDebugStreamLine("Back Left Limit: %d", SensorValue(BACK_L_LSWITCH_PORT));
	//writeDebugStreamLine("Back Right Limit: %d", SensorValue(BACK_R_LSWITCH_PORT));
#endif
	while (true)
	{
		delay(1000);
